...
# Note: this code is a lot of cut/paste from other code and very likely
# could be substantially improved/cleaned.
...
RE_MATHML_SPEECH = re.compile(
    ...
    r"|<phoneme alphabet='ipa' ph='(?P<ipa>[^']+)'> \
        (?P<phonemeText>[^ <]+)</phoneme> ?"
    # Prosody.
    r"|<prosody(?: pitch='(?P<pitch>\d+)%')?(?: volume='(?P<volume>\d+)%')? \
        (?: rate='(?P<rate>\d+)%')?> ?"
...
def getLanguageToUse(mathMl: str) -> str:
    """Get the language specified in a math tag if the language pref is Auto,
        else the language preference."""
    mathCATLanguageSetting = "Auto"
    try:
        # ignore regional differences if the
        # MathCAT language setting doesn't have it.
...
def ConvertSSMLTextForNVDA(text: str, language: str = "") -> list:
    ...
    # as of 7/23, oneCore voices do not implement the CharacterModeCommand
    # despite it being in supported_commands
    use_character = (
    ...
    for m in RE_MATHML_SPEECH.finditer(text):
        if m.lastgroup == "break":
            if use_break:
                out.append(BreakCommand(time=int(int(m.group("break")) *
                                                 breakMulti)))
        elif m.lastgroup == "char":
            ch = m.group("char")
            if use_character:
                out.extend(
                    (CharacterModeCommand(True), ch,
                ...
                out.extend(
                    (
                        " ",
                        "eigh" if ch == "a" and language.startswith("en")
                        else ch,
                        " ",
                    )
                )
            ...
            if command in supported_commands:
                out.append(command(
                    multiplier=int(m.group(m.lastgroup)) / 100.0))
            ...
            # MathCAT puts out spaces between words, the speak command
            # seems to want to glom the strings together at times,
...
class MathCATInteraction(mathPres.MathInteractionNVDAObject):
    # Put MathML on the clipboard using the two formats below
    # (defined by MathML spec)
    # We use both formats because some apps may only use one or the other
    # Note: filed https://github.com/nvaccess/nvda/issues/13240
    # to make this usable outside of MathCAT
    CF_MathML = windll.user32.RegisterClipboardFormatW("MathML")
    CF_MathML_Presentation = windll.user32.RegisterClipboardFormatW(
        "MathML Presentation"
    )
    # log.info("2**** MathCAT registering data formats: CF_MathML %x,
    # CF_MathML_Presentation %x" % (CF_MathML, CF_MathML_Presentation))

    def __init__(self, provider=None, mathMl: Optional[str] = None):
        super(MathCATInteraction, self).__init__(provider=provider,
                                                 mathMl=mathMl)
    ...
    def reportFocus(self):
        ...
        try:
            ...
            speech.speakMessage(
                _(
                    "Error in starting navigation of math:\
                         see NVDA error log for details"
                )
            )
    ...
    def script_navigate(self, gesture: KeyboardInputGesture):
        ...
        try:
            ...
            # update the braille to reflect the nav position
            # (might be excess code, but it works)
    ...
    def _copyToClipAsMathML(self, text: str, notify:
        ...
        try:
            with winUser.openClipboard(gui.mainFrame.Handle):
                ...
                self._setClipboardData(self.CF_MathML,
            ...
            if notify:
                ui.reportTextCopiedToClipboard()
                # No argument reports a failure.
            return False
        if got == text:
            if notify:
    ...
    def _setClipboardData(self, format, data: str):
        # Need to support MathML Presentation, so this copied from winUser.py
        # and the first two lines are commented out
        ...
        if not windll.user32.SetClipboardData(format, h):
            raise WinError()
        # NULL the global memory handle so that it is not freed
        #   at the end of scope as the clipboard now has it.
        h.forget()
...
class MathCAT(mathPres.MathPresentationProvider):
    def __init__(self):
        ...
        try:
            # IMPORTANT -- SetRulesDir must be the first call
            #   to libmathcat besides GetVersion()
            rules_dir = path.join(path.dirname(path.abspath(__file__)),
                                  "Rules")
            log.info(
                f"MathCAT {libmathcat.GetVersion()} \
                installed. Using rules dir: {rules_dir}"
            )
            ...
            # Translators: this message directs users to look in the log file
            speech.speakMessage(
                _("MathCAT initialization failed: \
                  see NVDA error log for details")
            )
        self._language = ""
...
def _monkeyPatchESpeak():
    ...
    currentSynth.speak = patched_speak.__get__(currentSynth,
                                               type(currentSynth))
...
def patched_speak(self, speechSequence: SpeechSequence):  # noqa: C901
    ...
    # We output malformed XML, as we might close an
    #   outer tag after opening an inner one; e.g.
    ...
    for item in speechSequence:
        if isinstance(item, str):
            ...
            if prosody:
                ...
                textList.append(
                    '<break time="1ms" />'
                )  # hack added for cutoff speech
                # (github.com/NSoiffer/MathCATForPython/issues/55)
            ...
            # We can't use str.translate because
            #   we want to reject unknown characters.
            try:
                phonemes = "".join([self.IPA_TO_ESPEAK[char]
                                    for char in item.ipa])
                # There needs to be a space after the phoneme command.
                # Otherwise, eSpeak will announce a subsequent
                #   SSML tag instead of processing it.
    ...
    text = "".join(textList)
    # log.info(f"\ntext={text}")
    # Added saving old rate and then resetting to that --
    #   work around for https://github.com/nvaccess/nvda/issues/15221
    # I'm not clear why this works since _set_rate()
    #   is called before the speech is finished speaking
    synth = getSynth()
