...
from .tokens import AliasToken, AnchorToken, BlockEndToken, BlockEntryToken, \
    ...
    FlowSequenceStartToken, KeyToken, ScalarToken, StreamEndToken, \
    TagToken, StreamStartToken
...
class Scanner:
    def __init__(self):
        ...
        #   self.forward(l=1)    # read the next l characters and move the pointer.  # noqa
    ...
    def fetch_block_entry(self):
        ...
        if not self.flow_level:
            ...
            if not self.allow_simple_key:
                raise ScannerError(
                    None, None, "sequence entries are not allowed here",
                    self.get_mark()
                )
    ...
    def fetch_key(self):
        ...
        if not self.flow_level:
            ...
            if not self.allow_simple_key:
                raise ScannerError(
                    None, None, "mapping keys are not allowed here",
                    self.get_mark()
                )
    ...
    def check_document_start(self):
        # DOCUMENT-START:   ^ '---' (' '|'\n')
        if self.column == 0:
            if self.prefix(3) == "---" and self.peek(3) \
                    in "\0 \t\r\n\x85\u2028\u2029":
                return True
    ...
    def scan_directive_name(self, start_mark):
        ...
        length = 0
        ch = self.peek(length)
        while "0" <= ch <= "9" or "A" <= ch <= "Z" \
                or "a" <= ch <= "z" or ch in "-_":
            length += 1
            ch = self.peek(length)
        if not length:
            raise ScannerError(
            ...
            if ch in "0123456789":
                ...
                if increment == 0:
                    raise ScannerError(
                        ...
                        start_mark,
                        "expected indentation indicator in the \
                            range 1-9, but found 0",
                        self.get_mark(),
                    )
                self.forward()
        ...
        while True:
            ...
            if not double and ch == "'" and self.peek(1) == "'":
                ...
                if ch in self.ESCAPE_REPLACEMENTS:
                    ...
                    self.scan_line_break()
                    chunks.extend(self.scan_flow_scalar_breaks(double,
                                                               start_mark))
                else:
                    raise ScannerError(
                        "while scanning a double-quoted scalar",
                ...
                if self.peek(k) not in "0123456789ABCDEFabcdef":
                    raise ScannerError(
                        ...
                        "expected URI escape sequence of 2 \
                            hexadecimal numbers, but found %r"
                        % self.peek(k),
                        self.get_mark(),
                    )
