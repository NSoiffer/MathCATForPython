...
from .events import NodeEvent, StreamStartEvent, StreamEndEvent, AliasEvent, \
    ScalarEvent, SequenceStartEvent, MappingStartEvent, SequenceEndEvent, \
    MappingEndEvent, DocumentStartEvent, CollectionStartEvent, \
    DocumentEndEvent, CollectionEndEvent
...
class Emitter:
    ...
    def prepare_version(self, version):
        major, minor = version
        if major != 1:
            raise EmitterError("unsupported YAML version: %d.%d" % (major,
                                                                    minor))
    ...
    def prepare_tag(self, tag):
        ...
        for prefix in prefixes:
            if tag.startswith(prefix) and (prefix == "!"
                                           or len(prefix) < len(tag)):
    ...
    def write_plain(self, text, split=True):
        ...
        while end <= len(text):
            ...
            if spaces:
                if ch != " ":
                    if start + 1 == end and self.column > self.best_width and split:  # noqa
                        self.write_indent()
                        self.whitespace = False
                        self.indention = False
                    else:
